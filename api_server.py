...
# FastAPI app
app = FastAPI(title="Fusefy AI Usecase Generator API", version="1.0.0")
...
class UsecaseRequest(BaseModel):
    ...
    s3url: str
    cloudId: str
    riskframeworkid: Optional[str] = None
...
# Import all functions from lambda_function.py
# (All the existing functions from your lambda code)
...
@app.get("/")
async def root():
    return {"message": "Fusefy AI Usecase Generator API", "version": "1.0.0"}
...
async def upload_document(
    ...
    cloud_id: str = Form(...),
    stage_name: str = Form(default="staging"),
    app_name: str = Form(default="fusefy"),
    risk_framework_id: Optional[str] = Form(default=None)
):
    """
    Upload and analyze a document to generate AI usecase assessment
    """
    try:
        # Validate file type
        ...
        extraction_result = extract_text_from_uploaded_file(file_content, file.filename)
        ...
        if "error" in usecase_data:
            raise HTTPException(status_code=500, detail=f"Error generating usecase: {usecase_data['error']}")
...
@app.post("/process-usecase", response_model=DocumentUploadResponse)
async def process_usecase_from_s3(request: UsecaseRequest):
    """
    Process usecase from S3 URL (existing lambda functionality)
    """
    try:
        # This would call the existing process_usecase_request function
        # For now, return a placeholder response
        return DocumentUploadResponse(
            success=True,
            message="S3 usecase processing not yet implemented in API version",
            error="Feature coming soon"
        )
    except Exception as e:
        logger.error(f"Error processing S3 usecase: {e}")
